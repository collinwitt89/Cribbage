...
.chips{display:flex;gap:7px;margin-bottom:14px;overflow-x:auto;padding-bottom:4px;scrollbar-width:none}
...
export default function App() {
  ...
  const [lbView, setLbView] = useState("all");
  ...
  const [soloEntries, setSoloEntries] = useState([{ player: "", score: "" }, { player: "", score: "" }]);
  const [teamEntries, setTeamEntries] = useState([{ players: ["", ""], score: "" }, { players: ["", ""], score: "" }]);
  ...
  function addPlayer() {
    ...
    if (!name) return;
    if (players.find(p => p.name.toLowerCase() === name.toLowerCase())) { alert("That name already exists."); return; }
  ...
  function addResult() {
    let entries;
    if (gameMode === "solo") {
      entries = soloEntries.filter(e => e.player).map(e => ({ players: [e.player], score: parseInt(e.score) || 0 }));
      if (entries.length < 2) { alert("Enter at least 2 players."); return; }
    } else {
      entries = teamEntries.map(t => ({ players: t.players.filter(Boolean), score: parseInt(t.score) || 0 }));
      for (const t of entries) { if (t.players.length === 0) { alert("Each team needs at least one player."); return; } }
    }
  ...
  function setSoloPlayer(i, val) { setSoloEntries(prev => prev.map((e, j) => j === i ? { ...e, player: val } : e)); }
  function setSoloScore(i, val) { setSoloEntries(prev => prev.map((e, j) => j === i ? { ...e, score: val } : e)); }
  function addSoloEntry() { setSoloEntries(prev => [...prev, { player: "", score: "" }]); }
  function removeSoloEntry(i) { setSoloEntries(prev => prev.filter((_, j) => j !== i)); }
  function setTeamPlayer(ti, pi, val) { setTeamEntries(prev => prev.map((t, i) => i !== ti ? t : { ...t, players: t.players.map((p, j) => j === pi ? val : p) })); }
  function setTeamScore(ti, val) { setTeamEntries(prev => prev.map((t, i) => i !== ti ? t : { ...t, score: val })); }
  ...
  if (histFilter !== "All") visGames = visGames.filter(g => g.entries.some(e => e.players.includes(histFilter)));
    ...
    <>
      ...
      <div className="app">
        <div className="z1">
          ...
          <nav className="nav">
            <button className={"nav-btn" + (tab === "lb" ? " on" : "")} onClick={() => setTab("lb")}><TrophyIcon /><span>Standings</span></button>
            <button className={"nav-btn" + (tab === "history" ? " on" : "")} onClick={() => setTab("history")}><HistoryIcon /><span>History</span></button>
            <button className={"nav-btn" + (tab === "roster" ? " on" : "")} onClick={() => setTab("roster")}><UserIcon /><span>Players</span></button>
          </nav>
          ...
          {tab === "lb" && (
            <div className="sec">
              ...
              {lb.length === 0 && <div className="empty">No results yet. Add players and record games to see standings.</div>}
              {lb.map((p, i) => (
                <div className="lb" key={p.name}>
                  <div className={"lb-rank" + (i < 3 && p.gp > 0 ? " top" : "")}>{i + 1}</div>
                  <div className="lb-name">{p.name}</div>
                  <div className="lb-stats">
                    <div className="lb-stat"><div className="lb-sv">{p.wins}</div><div className="lb-sl">W</div></div>
                    <div className="lb-stat"><div className="lb-sv">{p.losses}</div><div className="lb-sl">L</div></div>
                    <div className="lb-stat">
                      <div className={"lb-sv" + (p.net > 0 ? " pos" : p.net < 0 ? " neg" : "")}>{p.net > 0 ? "+" : ""}{p.net}</div>
          ...
          {tab === "history" && (
            <div className="sec">
              ...
              <div className="chips">
                {filterOpts.map(f => (
                  <button key={f} className={"chip" + (histFilter === f ? " on" : "")} onClick={() => setHistFilter(f)}>{f}</button>
                ))}
              </div>
              ...
              {visGames.map(game => {
                ...
                return (
                  <div className="card" key={game.id} style={{ cursor: "pointer" }} onClick={() => setExpanded(open ? null : game.id)}>
                    <div className="g-hdr">
                      <div className="g-meta">
                        <div className="g-date">{fmt(game.date)}</div>
                        <span className={"type-badge " + (game.mode === "solo" ? "type-solo" : "type-team")}>{game.mode === "solo" ? "Solo" : "Teams"}</span>
                      </div>
                      <ChevronDown />
                    </div>
                        ...
                        return (
                          <div key={ei}>
                            ...
                            <div className="g-entry">
                              <div className="g-entry-name">{entry.players.join(" & ")}{win && <span className="win-badge">Won</span>}</div>
                              <div className={"g-entry-score" + (win ? " win" : "")}>{entry.score}</div>
                            </div>
                          </div>
                        );
                      })}
                    </div>
                    {open && game.notes && <div className="g-notes">📝 {game.notes}</div>}
                  </div>
                );
              })}
            </div>
          ...
          {tab === "roster" && (
            <div className="sec">
              ...
              {players.length === 0 && <div className="empty">No players yet. Tap + to add players.</div>}
              {players.map(p => (
            ...
            <div className="modal" onClick={e => e.stopPropagation()}>
              ...
              <div className="choice">
                <button className="choice-btn" onClick={() => setModal("result")}>
                  ...
                  <div className="choice-txt"><strong>Game Result</strong><span>Record the outcome of a game just played</span></div>
                </button>
                <button className="choice-btn" onClick={() => setModal("player")}>
                  <div className="choice-ico"><UserIcon /></div>
                  <div className="choice-txt"><strong>New Player</strong><span>Add someone to the roster</span></div>
                </button>
              </div>
            </div>
          </div>
        ...
        {modal === "player" && (
          <div className="overlay" onClick={() => setModal(null)}>
            <div className="modal" onClick={e => e.stopPropagation()}>
              ...
              <input type="text" placeholder="e.g. Margaret" value={newName} onChange={e => setNewName(e.target.value)} onKeyDown={e => e.key === "Enter" && addPlayer()} autoFocus />
          ...
          <div className="overlay" onClick={() => setModal(null)}>
            <div className="modal" onClick={e => e.stopPropagation()}>
              ...
              <div className="mode-toggle">
                <button className={"mode-btn" + (gameMode === "solo" ? " on" : "")} onClick={() => setGameMode("solo")}><PersonIcon />Solo</button>
                <button className={"mode-btn" + (gameMode === "team" ? " on" : "")} onClick={() => setGameMode("team")}><UsersIcon />Teams</button>
              </div>
              <label>Date Played</label>
              <input type="date" value={gameDate} onChange={e => setGameDate(e.target.value)} />
              <div className="info">
                {gameMode === "solo" ? "Each player competed individually. Enter each player's final score." : "Two teams of two players. Each team shares a score."}
              </div>
              {gameMode === "solo" && (
                <>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 90px", gap: "0 10px", marginBottom: 4 }}>
                    <label>Player</label><label>Score</label>
                  </div>
                    ...
                    <div className="solo-row" key={i}>
                      <div>
                        {playerNames.length > 0 ? (
                          ...
                          <input type="text" placeholder={"Player " + (i + 1)} value={entry.player} onChange={e => setSoloPlayer(i, e.target.value)} />
                        )}
                      </div>
                      <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                        <input type="number" min="0" placeholder="0" value={entry.score} onChange={e => setSoloScore(i, e.target.value)} />
                        {soloEntries.length > 2 && (
                          <button onClick={() => removeSoloEntry(i)} style={{ background: "none", border: "none", color: "var(--cream-dim)", cursor: "pointer", fontSize: 18, lineHeight: 1, paddingBottom: 14, flexShrink: 0 }}>x</button>
                        )}
                      </div>
                    </div>
                  ))}
                  {soloEntries.length < 6 && <button className="link-btn" onClick={addSoloEntry}>+ Add another player</button>}
                </>
              )}
              {gameMode === "team" && (
                <>
                  {teamEntries.map((team, ti) => (
                    <div className="fgroup" key={ti}>
                      <div className="fgroup-title"><UsersIcon />Team {ti + 1}</div>
                      {[0, 1].map(pi => (
                        <div key={pi}>
                          ...
                          {playerNames.length > 0 ? (
                            ...
                            <input type="text" placeholder={"Player " + (pi + 1)} value={team.players[pi]} onChange={e => setTeamPlayer(ti, pi, e.target.value)} />
                          )}
                        </div>
                      ))}
                      <label>Team Score</label>
                      <input type="number" min="0" placeholder="e.g. 121" value={team.score} onChange={e => setTeamScore(ti, e.target.value)} style={{ marginBottom: 0 }} />
                    </div>
                  ))}
                </>
              )}
              ...
              <textarea placeholder="Anything noteworthy about this game?" value={gameNotes} onChange={e => setGameNotes(e.target.value)} />
              <div className="actions">
                <button className="btn btn-outline" onClick={() => setModal(null)}>Cancel</button>
